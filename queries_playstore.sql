...
SELECT * FROM analytics (id,name) WHERE last_updated = '2018-08-01';
...
SELECT * FROM analytics WHERE rating=5 LIMIT 5; --random top 5-
...
-- SELECT * FROM analytics (app_name,price,rating) WHERE rating<3 ORDER BY price DESC LIMIT 1;
SELECT app_name, price, rating FROM analytics WHERE rating<3 ORDER BY price DESC LIMIT 1;

-- 8. Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.
SELECT * FROM analytics WHERE min_installs<=50 AND rating IS NOT NULL ORDER BY reviews DESC;
...
SELECT * FROM analytics WHERE price>.10 AND price<1 ORDER BY reviews LIMIT 10;
